...
import { ScreenReadingWrapper } from "@/components/ScreenReadingWrapper";
...
const TarotReading = ({ linkedinData }: TarotReadingProps) => {
  ...
  const createSparkleEffect = (element: Element) => {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      sparkle.className =
        "absolute bg-mystic-gold rounded-full animate-sparkle";
    ...
    for (let i = 0; i < 8; i++) {
      const sparkle = document.createElement("div");
      sparkle.className =
        "absolute w-2 h-2 bg-mystic-gold rounded-full animate-sparkle";
      sparkle.style.left = `${Math.random() * 100}%`;
      sparkle.style.top = `${Math.random() * 100}%`;
  ...
  return (
    <div className="min-h-dvh bg-[#121212] relative">
      {/* <div className="fixed inset-0 bg-gradient-to-t from-zinc-900 from-0% via-slate-100 via-50% to-neutral-600 to-100% opacity-50"></div> */}
            ...
            {cards.map((card, index) => (
              ...
              >
                ...
                <div className="text-center space-y-3">
                  ...
                  <p className="text-green font-alegreya text-xl">
                    {card.subtitle}
                  </p>
                  {!card.isRevealed && (
                    <p className="text-lg text-muted-foreground flex items-center justify-center gap-2 font-display tracking-wide">
                ...
                {card.isRevealed && card.reading && (
                  <div className="bg-card/50 backdrop-blur-sm rounded-xl p-6 border-2 border-mystic-gold/30 max-w-sm relative">
                    ...
                    <div className="absolute -top-1 left-1/2 transform -translate-x-1/2 bg-background px-3">
                      <div className="font-cormorant text-mystic-gold text-sm">
                        ✧ PROPHECY ✧
                      </div>
                    </div>
          ...
          {isGeneratingReadings && (
            <div className="text-center space-y-6 bg-card/30 backdrop-blur-sm rounded-xl p-8 border-2 border-mystic-gold/40 max-w-2xl mx-auto relative animate-pulse">
              ...
              <p className="text-foreground/80 font-alegreya text-xl leading-relaxed">
                Ancient algorithms merge with cosmic wisdom as your digital
                essence is analyzed through the sacred patterns of the
                universe...
              </p>
            </div>
          )}
